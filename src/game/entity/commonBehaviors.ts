...
/**
 * 風ジャンプSEの最短呼び出し間隔（ミリ秒）
 */
const WIND_JUMP_SE_INTERVAL = 100
let lastWindJumpSETime = 0

/**
 * エンティティが画面内にいるかどうか判定
 */
function isOnScreen(entity: Entity): boolean {
  const parent = entity.parent
  if (!parent) return false

  // entityのスクリーン上の座標を計算
  const screenX = entity.x + parent.x
  const screenY = entity.y + parent.y

  // 画面内判定（少しマージンを持たせる）
  const margin = 32
  return (
    screenX >= -margin &&
    screenX <= SCREEN.WIDTH + margin &&
    screenY >= -margin &&
    screenY <= SCREEN.HEIGHT + margin
  )
}
...
export const CommonBehaviors = {
   ...
   *
   * - 左壁にぶつかる → 右向きに反転（vx反転、scale.x = -1）
   * - 右壁にぶつかる → 左向きに反転（vx反転、scale.x = 1）
   *
   ...
   *
   * - 床に着地している場合のみ崖をチェック
   * - 右側が崖 → 左向きに方向転換（vx負、scale.x = 1）
   * - 左側が崖 → 右向きに方向転換（vx正、scale.x = -1）
   *
