...
class StageScene extends Scene {
    ...
    constructor(game, stageNum, retry = false) {
        ...
        this.stageWidth =
            stageData.stages.reduce((s1, s2) => (s1[0].length > s2[0].length ? s1 : s2))[0].length *
            BLOCKSIZE;
        this.stageHeight =
            stageData.stages.reduce((s1, s2) => (s1.length > s2.length ? s1 : s2)).length *
            BLOCKSIZE;
        ...
        this.player = new Player(null, null, stageData.stages, hp, maxHp, hpBar).addChildTo(this);
        ...
        if (!retry && stageData.name) {
            const moveTime = 40;
            const waitTime = 120;
            const r1 = new RectActor("#0008", new Rectangle(320, 90, 320, 30)).addChildTo(this);
            const r2 = new RectActor("#0008", new Rectangle(-320, 60, 320, 30)).addChildTo(this);
            ...
            const makeStageNameText = (name, y, fontSize, letterSize) => {
                ...
                for (let i = 0; i < name.length; i++) {
                    ...
                    letter.update = () => {
                        if (delayTime <= letter.time && letter.time <= delayTime + moveTime) {
                            letter.y = easeOutExpo(y, y1, letter.time - delayTime, moveTime);
                            letter.alpha = easeLinear(0, 1, letter.time - delayTime, moveTime);
        ...
        if (DEBUG) {
            const text = new TextActor(this.player.state.currentState, 8, 8).addChildTo(this);
            text.color = "white";
            text.textAlign = "left";
    ...
    makeWind(vx) {
        ...
        // 元トルネード消去
        const vanishingWind = new Wind(wind.x, wind.y, 0, wind.stages).addChildTo(this);
