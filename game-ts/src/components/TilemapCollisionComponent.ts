...
export class TilemapCollisionComponent {
  ...
  private isWall(x: number, y: number): boolean {
    ...
    return (
      blockType === CollisionType.SOLID || blockType === CollisionType.PLATFORM
    );
  }
  ...
  stopAtLeftWall() {
    ...
    // 壁ブロックの右端に配置
    this.entity.x =
      (Math.floor(nextLeft / BLOCKSIZE) + 1) * BLOCKSIZE - this.entity.hitbox.x;
    this.entity.vx = 0;
  }
  ...
  stopAtUpWall() {
    ...
    // 壁ブロックの下端に配置
    this.entity.y =
      (Math.floor(nextTop / BLOCKSIZE) + 1) * BLOCKSIZE - this.entity.hitbox.y;
    this.entity.vy = 0;
  }
