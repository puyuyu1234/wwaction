...
class Entity extends SpriteActor {
    ...
    update() {
        ...
        // 壁以外判定
        const isBlockType = (x, y, flag) => Math.floor(getBlockType(x, y) / 100) * 100 == flag;
...

        {
            // ダメージブロック
            let result;
            if ((result = findMatchingPair(xList, yList, (x, y) => isBlockType(x, y, 100)))) {
                const x = result.x;
                const y = result.y;
                let bx = Math.floor(x / BLOCKSIZE),
                    by = Math.floor(y / BLOCKSIZE);
            ...
            // 水中
            if ((result = findMatchingPair(xList, yList, (x, y) => isBlockType(x, y, 200)))) {
...
class Gurasan extends Entity {
    constructor(x, y, stage) {
        ...
        this.on("goRightWall", () => (this.scaleX *= -1));
        this.on("hitWind", () => {
            const nasake = new Nasake(this.x, this.y, this.stages).addChildTo(this.parent);
            nasake.scaleX = this.vx * -2;
            nasake.vx = this.vx / 2;
            this.dispatch("spawn", nasake);
            const sunGlass = new SunGlass(this.x, this.y, -this.vx, this.stages).addChildTo(
                this.parent
            );
